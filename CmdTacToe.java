...
public class CmdTacToe {
    ...
    // Recommended to keep this 3 or below while using the computer-controlled P2, as calculation time expands polynomially
    ...
    static double epsilon = 0.20; // Epsilon
    static boolean suppress = false; // Whether to suppress the space labels
    static boolean numpad = false; // Whether to use numpad controls instead of the plain numeric controls
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        String epsilonString = patternInArray(args, "^--epsilon=([0-9]+.[0-9]+)$");
        if (!epsilonString.isEmpty()) { // Activates imperfect mode with epsilon override
            imperfect = true;
            try {
                epsilon = Double.parseDouble(epsilonString); // Large epsilon is, the more random the computer is
            } catch (Exception e) {}
        }
    ...
    public static void goComputer() throws InterruptedException {
        ...
        printBoard();
        prompt("The Computer placed an '" + playersMark.get(false) + "' mark on space " + (index + 1));
        Thread.sleep(1000);
        printWipe();
    }
    ...
    public static void printWipe() {
        /**
         * From https://www.delftstack.com/howto/java/java-clear-console/ Wipes
         * the console of all previous text
         */
        try {
    ...
    public static int minimax(String[] board, int depth, boolean currentPlayer) {
        ...
        if (isGameOver(board)) {
            if (doesPlayerWin(board, false)) {
                return 10 - depth; // This is good for the computer
            } else if (doesPlayerWin(board, true)) {
                return depth - 10; // This is bad for the computer
            } else {
        ...
        if (currentPlayer) { // Now minimizing
            ...
            for (int i : availableMoves(board)) { // Test each next move
                String[] testBoard = board.clone(); // Create unrelated copy
